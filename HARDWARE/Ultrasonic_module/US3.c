...
void US3_IO_Init(void)
{
		 ...
		 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1;				  //Trig-->PA.1 端口配置
		 GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; ; 		 //上拉输入
		 GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	//IO口速度为50MHz
		 GPIO_Init(GPIOA, &GPIO_InitStructure);					 //根据设定参数初始化GPIOC.8
	
		 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;	    		 //Echo-->PA.0 端口配置, 推挽输出
	   GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 	//推挽输出
	   GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	//IO口速度为50MHz
		 GPIO_Init(GPIOA, &GPIO_InitStructure);	  				 //推挽输出 ，IO口速度为50MHz
		 GPIO_ResetBits(GPIOA,GPIO_Pin_1); 						 //PB.0 输出低
}
/*
通用定时器5初始化
...
float US3_check(void)
{
		 ...
		 while(!Echo);  //当回响信号为零时等待
				 TIM_ITConfig(TIM5,TIM_IT_Update,ENABLE);//使能能指定的TIM中断
				 TIM_Cmd(TIM5,ENABLE);//使能TIM5外设
			while(Echo);
				 TIM_ITConfig(TIM5,TIM_IT_Update,DISABLE);//失能指定的TIM中断
				 TIM_Cmd(TIM5,DISABLE);//失能TIM5外设	  
